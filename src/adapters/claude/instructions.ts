...
import { claudeMdPath, homeClaudeMdPath } from '../../core/paths';
...
function instructionsTargetPath(scope: 'project' | 'global', outputRoot: string): string {
  return scope === 'global' ? homeClaudeMdPath(outputRoot) : claudeMdPath(outputRoot);
}

export async function syncClaudeInstructions(
  projectRoot: string,
  outputRoot: string,
  ...
  const targetPath = instructionsTargetPath(scope, outputRoot);
...
export async function hasClaudeInstructionsSynced(
  projectRoot: string,
  outputRoot: string,
  config: AgentStdConfig,
  scope: 'project' | 'global' = 'project',
