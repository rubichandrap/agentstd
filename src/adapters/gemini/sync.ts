import { syncGeminiMcpServers } from './mcp';
...
export async function sync(ctx: SyncContext): Promise<SyncResult> {
  ...
  changed.push(...(await syncGeminiMcpServers(outputRoot, ctx.config, operations, ctx.dryRun)));

  return { target: 'gemini', changed, warnings, operations };
}
