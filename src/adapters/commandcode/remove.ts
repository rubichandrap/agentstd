...
export async function remove(ctx: RemoveContext): Promise<RemoveResult> {
  ...
  if (!(await fileExists(filePath))) {
    return { target: 'commandcode', removed, warnings, operations };
  }
  ...
  try {
    ...
    removed.push('.commandcode/mcp.json');
  } catch (err) {
    warnings.push(`Failed to clean .commandcode/mcp.json: ${(err as Error).message}`);
  }
